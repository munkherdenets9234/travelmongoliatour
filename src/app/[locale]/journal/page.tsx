...
const CATEGORIES = ['all', 'guides', 'culture', 'tips', 'stories', 'food', 'adventure'] as const
...
export default async function JournalPage({ params, searchParams }: Props) {
  ...
  const { featured, items, hasMore } = await getArticles(locale, { category, page })
  ...
  const categoryLabel = (c: (typeof CATEGORIES)[number]) => (c === 'all' ? t.category_all : t.categories[c])

  return (
    <>
      <div className="text-center px-6 pt-12 pb-2">
        <div className="text-xs font-semibold tracking-[0.24em] uppercase text-olive">{t.hero_eyebrow}</div>
